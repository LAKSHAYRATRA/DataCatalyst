...
export default function AdminLanguageSubprojects() {
    ...
    async function toggleBoost(sub) {
        ...
        setSaving(sub._id);
        try {
            await patch(`/api/admin/languages/${sub._id}`, { isBoosted: newBoost });
            setSubprojects(prev => prev.map(s => s._id === sub._id ? { ...s, isBoosted: newBoost } : s));
            setSuccess(`Subproject ${newBoost ? "boosted & recommended" : "unboosted"} successfully.`);
    ...
    async function saveSubproject(e) {
        ...
        try {
            ...
            if (editingSubproject) {
                await patch(`/api/admin/languages/${editingSubproject._id}`, payload);
                setSuccess(`Subproject "${computedName}" updated successfully.`);
            } else {
                await postJson("/api/admin/languages", { ...payload, code });
                setSuccess(`Subproject "${computedName}" created successfully.`);
            }
            closeModal();
    ...
    async function toggleEnable(sub) {
        ...
        setSaving(sub._id);
        try {
            const data = await patch(`/api/admin/languages/${sub._id}`, { enabled: next });
            setSubprojects(prev => prev.map(l => l._id === sub._id ? data.language || { ...l, enabled: next } : l));
            setSuccess(`Subproject "${sub.name}" is now ${next ? "active" : "disabled"}.`);
            setTimeout(() => setSuccess(""), 3000);
        } catch (e) {
            setError(e.message);
        } finally {
    ...
    async function handleDelete(sub) {
        ...
        setSaving(sub._id);
        try {
            await del(`/api/admin/languages/${sub._id}`);
    ...
    async function handleRemoveCallContributor(u) {
        const result = await Swal.fire({
        ...
        try {
            await postJson(`/api/admin/languages/${summaryModalLang._id}/remove-contributor`, { userId: u._id });
            await Swal.fire({
                title: "Contributor Removed",
                text: "The user has been removed from this language.",
                icon: "success",
                background: "#18181b",
                color: "#ffffff"
            });
    ...
    async function handleResetCallContributor(u) {
        const result = await Swal.fire({
        ...
        try {
            await postJson(`/api/admin/languages/${summaryModalLang._id}/reset-contributor`, { userId: u._id });
            await Swal.fire({
        ...
        <div className="min-h-screen bg-neutral-950 text-white pt-16 md:pt-0 md:pl-64">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-12 space-y-6">
                ...
                >
                    ...
                    <span>Back to Call Languages</span>
                </Link>
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight text-white flex items-center gap-2.5">
                                ...
                                <span>{currentBaseLanguage} Subprojects</span>
                            </h1>
                        ...
                        <p className="text-sm text-neutral-400">
                            Create and manage live calling subprojects, speaking roles (e.g. Doctor vs Patient), hourly payout rates, and quotas under {currentBaseLanguage}.
                        </p>
                    </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 bg-neutral-800/60 border border-neutral-700/70 p-4 rounded-2xl">
                    <div className="flex items-center gap-3 flex-1 min-w-[280px] max-w-md">
                        <div className="relative w-full">
                            ...
                            <input
                                type="text"
                                placeholder={`Search subprojects in ${currentBaseLanguage}...`}
                                value={searchQuery}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 border border-dashed border-neutral-800 rounded-2xl bg-neutral-900/40">
                        ...
                        <h3 className="text-base font-bold text-neutral-300">
                            {showOnlyActive ? `No Active Subprojects in ${currentBaseLanguage}` : `No Subprojects Found in ${currentBaseLanguage}`}
                        </h3>
                        <p className="text-xs text-neutral-500 mt-1 max-w-sm mx-auto mb-4">
                            {showOnlyActive 
                                ? `There are currently no active subprojects enabled under ${currentBaseLanguage}. Click below to add one.`
                                : `Get started by creating your first subproject (e.g. Doctor-Patient Conversations) in ${currentBaseLanguage}.`
                            }
                        </p>
                        <button
                            onClick={openAddModal}
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                                <span className="text-[11px] text-primary-400 font-semibold block">
                                                    Subproject in {sub.language || currentBaseLanguage}
                                                </span>
                                                {sub.enableCallRoles ? (
                        ...
                        <div className="p-5 border-b border-neutral-800 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold text-white">
                                    {editingSubproject ? `Edit Subproject (${editingSubproject.name})` : `Add Subproject under ${currentBaseLanguage}`}
                                </h2>
                            </div>
                        ...
                        <form onSubmit={saveSubproject} className="p-6 space-y-4 overflow-y-auto custom-scrollbar">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. Doctor-Patient Conversations, Hotel Booking, Technical Support..."
                                    required
                            ...
                            <div className="flex items-center gap-2.5 pt-1">
                                <input
                                    type="checkbox"
                                    id="boost-call-subproject-checkbox"
                                ...
                                />
                                <label htmlFor="boost-call-subproject-checkbox" className="text-xs font-bold text-amber-300 select-none cursor-pointer flex items-center gap-1.5">
                                ...
                                >
                                    {modalSaving ? (
                                        ...
                                        <>
                                            ...
                                            <span>{editingSubproject ? "Save Changes" : "Create Subproject"}</span>
                                        </>
                                    )}
                                </button>
                            </div>
                    ...
                    <div className="relative bg-neutral-800 border border-neutral-700 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in text-white">
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-700 bg-neutral-850">
                            <div>
                                ...
                                <p className="text-xs text-neutral-400 mt-0.5">Call Subproject: {summaryModalLang.name} ({summaryModalLang.code})</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <button
