...
import { HelpCircle, MessageSquare, Play, X, Check, Loader2, UserCheck } from 'lucide-react';
...
export default function RoleSelector({
    ...
    const r1 = role1Name || "Role 1";
    const r2 = role2Name || "Role 2";
        ...
        <div className="bg-neutral-900/90 border border-neutral-800 rounded-3xl p-6 md:p-8 shadow-xl backdrop-blur-xl animate-scale-in text-white space-y-6">
            <div>
                ...
                <p className="text-neutral-400 text-xs mt-1">
                    Select your speaking role for this conversation ({r1} vs {r2}).
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <button
                    type="button"
                    onClick={() => onSelectRole(r1)}
                    disabled={myRole !== null || peerRole === r1}
                ...
                >
                    <div>
                        ...
                        <div className="text-lg font-black text-white mb-1">{r1}</div>
                        <div className="text-xs text-neutral-400">Lead this part of the conversation</div>
                    </div>
                ...
                <button
                    type="button"
                    onClick={() => onSelectRole(r2)}
                    disabled={myRole !== null || peerRole === r2}
                    className={`p-6 rounded-2xl border-2 transition-all relative overflow-hidden flex flex-col justify-between text-left cursor-pointer group ${
                        myRole === r2
                ...
                >
                    <div>
                        ...
                        <div className="text-lg font-black text-white mb-1">{r2}</div>
                        <div className="text-xs text-neutral-400">Respond and dialogue as {r2}</div>
                    </div>
            ...
            <div className="bg-neutral-950 border border-neutral-800 p-5 rounded-2xl text-center shadow-inner space-y-3">
                ...
                {!myRole && peerRole && (
                    <div className="text-indigo-300 text-xs font-semibold animate-pulse">
                        Partner selected <span className="font-bold text-white">{peerRole}</span>. Please click <span className="font-bold text-white">{peerRole === r1 ? r2 : r1}</span> to confirm!
                    </div>
                )}
                {myRole && peerRole && (
                    <div className="flex flex-col items-center space-y-3">
                        <div className="text-emerald-400 text-xs font-extrabold flex items-center gap-1.5 uppercase tracking-wider">
                            ...
                            <span>Roles Confirmed ({myRole} &amp; {peerRole})! Ready to begin</span>
                        </div>
                        <button
                            type="button"
                            onClick={onStartCall}
